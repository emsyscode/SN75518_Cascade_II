...
#define VFD_LEof74LS237 11 // This is the pin number 11 on Arduino UNO
...
void send_char(unsigned char a) {
  ...
  // the validation of data happen when clk go from LOW to HIGH.
   // When strobe is low, all output is enable. If high, all output will be set to low.
  validBegin();
  delayMicroseconds(5);
...
void allON_SN75518_grid_2() {
  ...
  send_char_without(0B10000000);  send_char_without(0B00000000); //   7:0   -   15:8//    byte 0 & 1
...
for (uint8_t n = 0; n<2; n++){
      for (int i = 0; i<32; i++){
        ...
        animation_clock(word0,word1,word2,word3,word4,word5,word6,word7);
        //Uncomment the next 4 lines to get data of array at binnary values!
...
//
//        //Uncomment the next 4 lines to get data of array at hexadecimal values!
//        Serial.print(cnt, DEC); Serial.print(" : ");Serial.print(" 0x");
//        Serial.print(word0, HEX); Serial.print(word1, HEX);Serial.print(word2, HEX);Serial.print(word3, HEX);
//        Serial.print(" ; 0x");
//        Serial.print(word4, HEX); Serial.print(word5, HEX); Serial.print(word6, HEX);Serial.println(word7, HEX);  
//
        cnt++;
        validFinish();
        delay(500);
...
void validBegin(){
digitalWrite(VFD_LEof74LS237, LOW);      //11 /LE of 74LS237  
digitalWrite(VFD_in, LOW);      // 7 A0 of 74LS237
digitalWrite(VFD_stb1, LOW);   // 9 A1 of 74LS237
digitalWrite(VFD_stb2, HIGH);    //10 A2 of 74LS237 This combination of A0~A2 active the port Y4
delay(2);
digitalWrite(VFD_LEof74LS237, HIGH);      //11 /LE of 74LS237 // This keep the activation of port Y4
}
void validFinish(){
digitalWrite(VFD_LEof74LS237, LOW);      //11 /LE of 74LS237  
digitalWrite(VFD_in, LOW);      // 7 A0 of 74LS237
digitalWrite(VFD_stb1, LOW);   // 9 A1 of 74LS237
digitalWrite(VFD_stb2, LOW);    //10 A2 of 74LS237 This combination set port Y4 to LOW
delay(2);
digitalWrite(VFD_LEof74LS237, LOW);      //11 /LE of 74LS237 This allow the open the latch and pass the values to output of SN75518
}
void setup() {
  pinMode(VFD_in, OUTPUT); //7 of Arduino
  pinMode(VFD_clk, OUTPUT); //8 of Arduino
  pinMode(VFD_stb1, OUTPUT); //9 of Arduino
  pinMode(VFD_stb2, OUTPUT); //10 of Arduino
  pinMode(VFD_LEof74LS237, OUTPUT); //11 of Arduino
  //pinMode(VFD_latchBoth, OUTPUT);

  pinMode(ledPin, OUTPUT); // sets the digital pin as output
  Serial.begin(115200); // only to debug
  
//Here we do a test of the initialization of SN74LS237 to 
//set the output fix as Y4. The Y4 is responsable to control the
//pins of Latch's and Strob's of both IC's SN75518.
digitalWrite(VFD_LEof74LS237, LOW); //11 /LE of 74LS237
digitalWrite(VFD_in, LOW);          // 7 A0 of 74LS237
digitalWrite(VFD_stb1, LOW);        // 9 A1 of 74LS237
digitalWrite(VFD_stb2, HIGH);       //10 A2 of 74LS237
delay(2);
digitalWrite(VFD_LEof74LS237, HIGH);//11 /LE of 74LS237
for(int y =0; y<10; y++){
  delay(100);
...
void loop() {
       ...
       digitalWrite(ledPin, HIGH); // sets the LED on
        ...
        msgHELLO_grid_1(); //Static message!
        delay(1000); // waits for one second
        allOFF_SN75518(); // All off
        //
          for (unsigned int v=0; v < 50; v++){  //Used as dynamic message VFD control.
          msgPEOPLE_grid_1();
          delay(5); 
          msgPEOPLE_grid_2();
          delay(5); 
          }
//findSegments();  //Used to debug, if is not at use comment all lines related to interrupt and trigger events!
    testClock();
    delay(1000);
}
/*****************************************  ISR *************************************/
